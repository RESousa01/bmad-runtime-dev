---
title: "Worker Images and Command DSL"
aliases:
  - "38 - Worker Images and Command DSL"
tags:
  - bmad-runtime
...
# Worker Images and Command DSL

## V6.17 scope and local counterpart

Worker images, fixed templates, container entrypoints, Blob checkouts, and Python command implementations in this note apply only to `web_managed` and explicit remote-job records. The desktop runner is a separate Rust/Win32 implementation with a Windows-specific candidate and result manifest in [[95 - Windows Local Workspace and Execution]].

Both use structured `argv[]` and shared action classes, but their audiences, executable identity, environment binding, containment claims, storage, and result types differ. `sealed_test_fake` is the name for the deterministic development adapter. A Job Object cannot be used as proof of filesystem or network confinement.
...
| Field | Value |
|---|---|
| Component | `Worker Image and Command DSL Reference` |
| Area | `execution safety` |
| Primary implementation package | `workers/python-executor` |
| Runtime/technology | `Python + JSON schemas` |
| First-slice priority | `after-core or supporting` |


## 2. Purpose

Specify worker image layout, entrypoints, command DSL, patch DSL, environment handling, and result manifest protocol.
...
6. For `Worker Image and Command DSL Reference`, implement only the smallest behavior that proves its contract in the first executable slice, then add extended BMAD/Builder/artifact behavior after gate approval.
...
- Commands targeting the platform's own lifecycle (stopping/restarting the Runtime API, job infrastructure, or the worker's own supervisor; `pkill`-style self-targeting) are denied as a distinct command class, checked both when a spec is created and when it executes. Denylist patterns anchor on concrete command shapes (executable + operand), never on prose keywords, so descriptions mentioning "restart" do not false-positive (pattern proven in Hermes `cron/lifecycle_guard.py`).
...
`38 - Worker Images and Command DSL.md` is part of the implementation library support layer. In v3, support files were useful but not always testable. In v4, every support file must provide either a decision, reference contract, release gate, mapping, runbook, or checklist that can be executed by a developer or coding agent.
...
## V6.16 remote-build and fixed-template requirements

- No local Docker or Kubernetes workflow is required or supported as a release gate. Local tests use the non-isolating deterministic fake described in [[36 - Local Development and DevEx]].
- ACR Tasks or hosted CI builds each worker from an immutable source revision and lockfiles, records component licenses, vulnerability results, SBOM, provenance/attestation, signature, and output digest, then runs a fixed-entrypoint smoke in Azure.
- A worker image is per-purpose and least-privileged; Python/runtime versions and native dependencies are locked per image, not assumed globally.
- ACA IaC fixes image digest, entrypoint, identity, secret refs, network profile, resource limits, timeout/retry, and output prefixes. Runtime input may choose only an allowlisted template and typed operation; it cannot override those fields.
- `WebWorkerResultManifest` is the canonical output. It binds candidate/spec/policy/approval, owner/run/attempt/lease, template/image/workspace/mutable inputs, command, output hashes, completion nonce, and bounded/redacted logs.
- Imported/generated code, dependency restores, package rehearsal, shell, and arbitrary workspace commands are never sent to `sealed_test_fake`. Web execution begins only in the approved Azure lane.
