---
title: "AI Coding Agent Handoff Prompts"
aliases:
...
# AI Coding Agent Handoff Prompts

## V6.17 mandatory work-packet header

Every implementation prompt declares `deliveryModel: shared | web_managed | windows_local`, authority owner, writable stores, executor audience, prohibited dependencies/edges, source contracts, security gate, fixtures, and required evidence. Prompts must not ask one agent to implement both execution models in a common adapter.

Desktop packets explicitly forbid generic Tauri filesystem/shell bridges, Azure-backed local lifecycle, raw shell strings, provider keys on device, direct remote apply, multi-file atomicity claims, and Job-Object-as-sandbox claims. Web packets explicitly preserve cloud snapshot and fixed remote isolation. Shared packets are limited to schemas, fixtures, BMAD semantics, UI primitives, and conformance—not authority code.
...
For LLM-specific work packet structure, development modes, TDD guidance, prompt patterns, and phase rules, use [[90 - LLM-Tailored Development Plan and Agent Workflow]] before handing work to an AI coding agent.

Every AI handoff must also include story size, stop conditions, rollback or disable path, observability impact, and expected context ledger output. A prompt that cannot name those fields is not ready for implementation.
...
| Field | Value |
|---|---|
| Component | `AI Coding Agent Handoff Prompts` |
| Area | `implementation handoff` |
| Primary implementation package | `docs/prompts` |
...
Provide concise but strict prompts for coding agents implementing each phase without losing project constraints.
...
### LLM-specific execution rule

Every coding-agent prompt must identify the development mode from [[90 - LLM-Tailored Development Plan and Agent Workflow]]:
...
- refactor-with-shim development.

If the prompt cannot name the mode, the work packet is not ready.

Every coding-agent prompt must also state:

- Story size from XS through XL.
- Stop conditions that require the agent to pause instead of guessing.
- Rollback, disable, or quarantine path for risky changes.
- Observability impact for new success, failure, degraded, or denied states.
- Required context ledger entries for the final report.
- Tests that must be written or updated before risky implementation.
...
6. For `AI Coding Agent Handoff Prompts`, implement only the smallest behavior that proves its contract in the first executable slice, then add extended BMAD/Builder/artifact behavior after gate approval.
...
`44 - AI Coding Agent Handoff Prompts.md` is part of the implementation library support layer. In v3, support files were useful but not always testable. In v4, every support file must provide either a decision, reference contract, release gate, mapping, runbook, or checklist that can be executed by a developer or coding agent.
