...
import { buildSchemaClosureManifest } from "./lib/schema-closure.mjs";
...
const sealedHelpClosureRootIds = Object.freeze([
...
const schemaClosureManifests = Object.freeze(sealedHelpClosureRootIds.map((rootSchemaId) =>
  buildSchemaClosureManifest({ rootSchemaId, schemas })));
const schemaClosureMap = Object.freeze(Object.fromEntries(
  schemaClosureManifests.map((manifest) => [manifest.rootSchemaId, manifest]),
));
const schemaClosureRuntime = `
...
export const SCHEMA_CLOSURES = Object.freeze(Object.fromEntries(
  Object.entries(${JSON.stringify(schemaClosureMap, null, 2)})
    .map(([schemaId, manifest]) => [schemaId, freezeSchemaClosure(manifest)]),
));
`;
add("generated/typescript/runtime.mjs", `${typescriptRuntime}${schemaClosureRuntime}`);

if (!typescriptOnly) {
  const closureConstants = schemaClosureManifests.map((manifest) => {
    const name = manifest.rootSchemaId
      .slice(manifest.rootSchemaId.lastIndexOf("/") + 1, -".schema.json".length)
      .replaceAll("-", "_")
      .toUpperCase() + "_SCHEMA_CLOSURE_SHA256";
    return { name, value: manifest.closureSha256 };
  });
  const rustPath = "generated/rust/contracts.rs";
  add(rustPath, `${expectedFiles.get(rustPath)}\n// BMAD sealed Help schema closures.\n${closureConstants
    .map(({ name, value }) =>
      `pub const ${name}: &str =\n    ${JSON.stringify(value)};`)
    .join("\n")}\n`);
  add("generated/dotnet/BmadSchemaClosures.g.cs", `//------------------------------------------------------------------------------
// <auto-generated>
...
{
${closureConstants.map(({ name, value }) =>
    `    public const string ${name} = ${JSON.stringify(value)};`).join("\n")}
}
`);
}
...
const canonicalJsonSource = await readContainedUtf8File(
  packageRoot,
  path.join(packageRoot, "scripts", "lib", "canonical-json.mjs"),
  "canonical JSON generator source",
);
const sha256Source = await readContainedUtf8File(
  packageRoot,
  path.join(packageRoot, "scripts", "lib", "sha256.mjs"),
  "SHA-256 generator source",
);
const browserCanonicalJson =
  `// @generated from scripts/lib/canonical-json.mjs; DO NOT EDIT.\n${canonicalJsonSource}`;
const browserSha256 =
  `// @generated from scripts/lib/sha256.mjs; DO NOT EDIT.\n${sha256Source}`;
...
add("generated/typescript/canonical-json.mjs", browserCanonicalJson);
add("generated/typescript/sha256.mjs", browserSha256);
...
if (!typescriptOnly) {
  ...
  schemaClosures: schemaClosureManifests,
  schemaTree: {
