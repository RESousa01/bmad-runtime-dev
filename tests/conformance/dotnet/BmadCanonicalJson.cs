...
public static class BmadCanonicalJson
{
    public static string Serialize(JsonElement value, string? excludedRootField = null)
    {
        var builder = new StringBuilder();
        AppendValue(builder, value, excludedRootField, true);
        return builder.ToString();
    }

    public static string HashWithoutField(
        string purpose,
        string schemaMajor,
        JsonElement value,
        string excludedRootField)
    {
        string canonical = Serialize(value, excludedRootField);
        return HashCanonical(purpose, schemaMajor, canonical);
    }
    ...
    private static string HashCanonical(string purpose, string schemaMajor, string canonical)
    {
        ...
        bool isRoot)
    {
        switch (value.ValueKind)
        {
            case JsonValueKind.Object:
                AppendObject(builder, value, isRoot ? excludedRootField : null);
                break;
        ...
        string? excludedField)
    {
        JsonProperty[] properties = value
            .EnumerateObject()
            .Where(property => !StringComparer.Ordinal.Equals(property.Name, excludedField))
        ...
        for (int index = 0; index < properties.Length; index++)
        {
            ...
            builder.Append(':');
            AppendValue(builder, properties[index].Value, null, false);
        }
    ...
    private static void AppendArray(StringBuilder builder, JsonElement value)
    {
        ...
        foreach (JsonElement item in value.EnumerateArray())
        {
            ...
            AppendValue(builder, item, null, false);
        }
