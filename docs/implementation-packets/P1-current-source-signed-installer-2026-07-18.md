...
### Main-merge review ledger

- Local merge `8545bcf6` preserved the reviewed P1 tree byte-for-byte, but independent post-merge review found that the annotated `v2` tag-object hash had been mistaken for an executable action commit. The build-provenance action is corrected to the official peeled `v2.4.0` commit `e8998f949152b193b063cb0ec769d69d929409be`, verified against the upstream ref and raw `action.yml`; the boundary policy now allowlists that exact commit.
- The same review closed two release-proof gaps: the unsigned dry-run lane now pins every third-party action to an immutable commit, and signed upgrade qualification requires canonical SemVer with `prior < current`, including prerelease ordering, before installer execution. Fresh-install qualification remains valid without prior-version inputs.
- Focused correction proof: release regressions 23/23, boundary policy, PowerShell AST, direct PowerShell SemVer/fresh-install probes, and `git diff --check` green. Independent re-review reports no remaining P0/P1 blockers. A durable cross-platform PowerShell comparator harness and consolidation of the metadata resolver's older permissive version regex remain non-blocking follow-ups.
